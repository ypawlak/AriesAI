...
class BoardStateBuilder {
    ...
    static public BoardStateBuilder startFromString(String str)
    {
        ...
        {
            throw new IllegalArgumentException(String.format("Building from string - improper number of fields for board: %s", noFields));
        }
        ...
        for(int y=0; y<size; y++)
        {
            for(int x=0; x<size; x++)
            {
                ...
                {
                    throw new IllegalArgumentException(String.format("Building from string -- illegal character encoding field: %s", chars[i]));
                }
